...
import { TestRunResult, TDDPhase, CoverageReport } from './tddTypes.js';
    ...
    switch (framework.toLowerCase()) {
      case 'jest': {
        ...
        if (parsed.testResults) {
          result.failures = parsed.testResults.flatMap((suite: any) =>
...
async function parseCoverageReport(_framework: string): Promise<CoverageReport | undefined> {
  try {
